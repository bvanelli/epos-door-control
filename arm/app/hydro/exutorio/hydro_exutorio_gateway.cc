...
int http_send()
{
    ...
    // cout << "...Sending turbidity_OBS_high db_series..." << endl;
    // attach_payload.payload(turbidity_OBS_high->db_series());
    // do {
    //     check_timeout();
    //     ret = Quectel_HTTP::post("https://150.162.62.3/api/attach.php", &attach_payload, sizeof(Attach_Payload));    
    //     cout << "post turbidity_OBS_high db_series: " << ret << endl;
    // } while(ret <= 0);
    // ret = 0;

    cout << "...Sending turbidity_OBS_low db_series..." << endl;
    attach_payload.payload(turbidity_OBS_low->db_series());
    do {
    check_timeout();
        ret = Quectel_HTTP::post("https://150.162.62.3/api/attach.php", &attach_payload, sizeof(Attach_Payload));    
        cout << "post turbidity_OBS_low db_series: " << ret << endl;
    } while(ret <= 0);
    ret = 0;
...
int main()
{
    ...
    // turbidity_OBS_high = new Water_Turbidity_OBS(0, INTEREST_EXPIRY, Water_Turbidity_OBS::PRIVATE);
